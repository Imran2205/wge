...
class Graph(object):
    def __init__(self, triples):
        ...
        def freeze(d):
            frozen = {}
            for key, subdict in list(d.items()):
                frozen[key] = {}
                for subkey, set_val in list(subdict.items()):
    ...
    def shortest_path(self, source, target):
        ...
        while len(queue) != 0:
            current = queue.pop()
            for rel, nbrs in list(self.neighbors[current].items()):
                for nbr in nbrs:
    ...
    def relation_stats(self):
        ...
        for r, args in list(self.relation_args.items()):
